...
#include <stdlib.h>
    ...
    void update_npool(int *clyr, double *amt, double *frac_nh4,
                      ...
                      char *subname, long int strlen)
    {
      ...
      no3amt = *frac_no3 * (*amt);
      if (debug) {
        fprintf(stdout, "\nUPDATE %s: clyr=%1d: amt = %12.10lf\n", subname,
                *clyr, *amt);
        fprintf(stdout, "UPDATE %s: no3amt = %12.10lf\t frac_no3 = %12.10lf\n", 
                subname, no3amt, *frac_no3);
        fprintf(stdout, "UPDATE %s: nh4amt = %12.10lf\t frac_nh4 = %12.10lf\n", 
      ...
      *ammonium += nh4amt;
      ...
      if (*frac_no3 > 0.0) {
        if ((*clyr == 1) && (no3amt >= 0.0)) {
          ...
          for(ilyr = layers->ubnd[*clyr-1]; ilyr <= layers->lbnd[*clyr-1];
            ...
            if (debug > 1) {
              fprintf(stdout, "nitrate[%1d] = %12.10lf\n", ilyr,
                      nitrate[ilyr]);
            }
            if (nitrate[ilyr] > 0.0) {
          ...
          if (debug) {
            fprintf(stdout, "no3_clyr_psum = %12.10lf\n", no3_clyr_psum);
          }
          for(ilyr = layers->ubnd[*clyr-1]; ilyr <= layers->lbnd[*clyr-1];
              ilyr++) {
            if (no3amt >= 0.0) {
              ...
              nitrate[ilyr] += ((layers->width[ilyr])/tdepth)*no3amt;
              cum_no3 += ((layers->width[ilyr])/tdepth)*no3amt;
            } else {
              /* ADD or REMOVE nitrate */
              if (nitrate[ilyr] > 0.0) {
                if (debug) {
                  fprintf(stdout, "   %1d: %12.10lf + %12.10lf = ", ilyr,
                          nitrate[ilyr],(nitrate[ilyr]/no3_clyr_psum)*no3amt);
                }
                cum_no3 += (nitrate[ilyr]/no3_clyr_psum) * no3amt;
                nitrate[ilyr] += (nitrate[ilyr]/no3_clyr_psum) * no3amt;
                if (debug) {
                  fprintf(stdout, "%12.10lf\n", nitrate[ilyr]);
                }
              }
            }
          }
      ...
      if (debug) {
        fprintf(stdout, "In update_npool: nh4amt = %12.10lf\n", nh4amt); 
        fprintf(stdout, "In update_npool: cum_no3 = %12.10lf\n", cum_no3); 
        fprintf(stdout, "In update_npool: cum_tot = %12.10lf\n", cum_tot); 
      }

      if (fabs(cum_no3-no3amt) > 1.0E-9) {
        fprintf(stdout, "Error in update_npool: %s.  no3amt = %12.10lf,  ",
                subname, no3amt);
        fprintf(stdout, "cum_no3 = %12.10lf\n", cum_no3); 
        exit(1);
      }
