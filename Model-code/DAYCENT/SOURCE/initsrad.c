...
YEARDATA_SPT yeardata = &Year_Data;  /* radiation, and day length */
                  ...
                  double daylength[NDAY])
    {
      ...
      for (ii=0 ; ii<365 ; ii++) {
        ...
        decl = MINDECL * cos((ii + DAYSOFF) * RADPERDAY);
        cosdecl = cos(decl);
        sindecl = sin(decl);
        ...
        daylength[ii] = yeardata->daylength[ii] / SEC_PER_HOUR;

        /* solar constant as a function of yearday (W/m^2) */
        sc = 1368.0 + 45.5*sin((2.0*PI*ii/365.25) + 1.7);
      ...
      daylength[365] = yeardata->daylength[365] / SEC_PER_HOUR;
    }
