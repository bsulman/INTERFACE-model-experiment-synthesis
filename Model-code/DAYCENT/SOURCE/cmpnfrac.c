...
#include "swconst.h"
    ...
    void cmpnfrac(int *clyr, double *ammonium, double nitrate[],
                  double minerl[], double *frac_nh4, double *frac_no3)
    {
      int iel;    /* Nitrogen */
      double Nsum;
...
/*          *frac_nh4 = *ammonium/(minerl[(iel)*CENTMAXLYR + (*clyr-1)]); */
      ...
      if ((*frac_nh4 > 1.0 + 1.0E-04) || (*frac_nh4 < 0.0 - 1.0E-04)) {
        fprintf(stderr, "Error in cmpnfrac, frac_nh4 = %12.10lf\n", *frac_nh4);
        fprintf(stderr, "clyr = %1d\n", *clyr);
        fprintf(stderr, "ammonium = %12.10lf\n", *ammonium);
        fprintf(stderr, "Nsum = %12.10lf\n", Nsum);
        fprintf(stderr, "minerl[%1d][1] = %12.10lf\n",
