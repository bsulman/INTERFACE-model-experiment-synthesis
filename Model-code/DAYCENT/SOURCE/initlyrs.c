...
#include "n2o_model.h"
                  ...
                  SITEPAR_SPT sitepar)
    {
      ...
      ilyr=0;
      while(fscanf(fp_in, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
      ...
      if (avgsand > 0.7) {
        ...
        printf("Error in initlyrs, unknown texture, sand = %lf\n", avgsand);
        exit(1);
      }
      ...
      if (flags->verbose) {
        ...
        for (ilyr=0; ilyr < layers->numlyrs; ilyr++) {
          printf("%3d\t%5.1lf\t%5.1lf\t%4.2lf\t%6.4lf\t%6.4lf\t",
                 ilyr, layers->dpthmn[ilyr], layers->dpthmx[ilyr], 
                 layers->bulkd[ilyr], layers->fieldc[ilyr],
                 layers->wiltpt[ilyr]); 
          printf("%4.2lf\t%4.2lf\t%4.2lf\t%4.2lf\t%4.2lf\t%4.2lf\t%7.5lf\t%4.2lf\n", 
        ...
        for (ilyr=0; ilyr < layers->numlyrs; ilyr++) {
          printf("%3d\t%5.1lf\t%5.1lf\t%7.3lf\t%7.3lf\t%6.3lf\t%6.4lf\n", ilyr, 
