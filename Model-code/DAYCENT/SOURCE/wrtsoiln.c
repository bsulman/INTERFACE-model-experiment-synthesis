...
#include "soilwater.h"
    ...
    void wrtsoiln(double *time, int *jday, double *ammonium, double nitrate[])
    {
      ...
      fprintf(files->fp_soiln, "%8.2lf,%4d", *time, *jday);
      fprintf(files->fp_soiln, ",%12.6lf", NH4_ppm);
      ...
      for (ilyr=0; ilyr < layers->numlyrs; ilyr ++) {
        ...
        fprintf(files->fp_soiln, ",%12.6lf", NO3_ppm);
      }
