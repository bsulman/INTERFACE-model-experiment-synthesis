...
#include "soilwater.h"
    ...
    void bal_npool(int *nlayer, double minerl[], double *ammonium,
                   double nitrate[], double *inorglch)
    {
      ...
      if (fabs(npool_sum - minerl_sum) > 1.0E-5) {
        if (debug) {
          ...
          fprintf(stdout, "npool_sum = %13.10lf   minerl_sum = %13.10lf  ",
                  npool_sum, minerl_sum);
          fprintf(stdout, "npool-minrl = %13.10lf\n ", npool_sum - minerl_sum);
          fprintf(stderr, "Nitrogen soil pools are unbalanced.\n");
          fprintf(stderr, "npool_sum = %13.10lf   minerl_sum = %13.10lf  ",
                  npool_sum, minerl_sum);
          fprintf(stderr, "npool-minrl = %13.10lf\n ", npool_sum - minerl_sum);
          for (clyr=0; clyr <= *nlayer; clyr++) {
            fprintf(stdout, "minerl[%1d][N] = %13.10lf\n", clyr+1,  
                    minerl[(iel)*CENTMAXLYR + (clyr)]);
            fprintf(stderr, "minerl[%1d][N] = %13.10lf\n", clyr+1,  
                    minerl[(iel)*CENTMAXLYR + (clyr)]);
          }
          for (ilyr=0; ilyr <= layers->numlyrs; ilyr ++) {
            fprintf(stdout, "nitrate[%1d] = %13.10lf\n", ilyr, nitrate[ilyr]);
            fprintf(stderr, "nitrate[%1d] = %13.10lf\n", ilyr, nitrate[ilyr]);
          }
          fprintf(stdout, "ammonium = %13.10lf\n", *ammonium);
          fprintf(stderr, "ammonium = %13.10lf\n", *ammonium);
        }
...
/*          exit(1); */
      ...
      for (clyr=0; clyr < CENTMAXLYR; clyr++) {
        minerl[(iel)*CENTMAXLYR + (clyr)] = 0.0l;
      }

      /* minerl(1,N) = ammonium */
      clyr = 0;
      minerl[(iel)*CENTMAXLYR+(clyr)] = *ammonium;

      for (clyr=0; clyr < *nlayer; clyr++) {
        for(ilyr = layers->ubnd[clyr]; ilyr <= layers->lbnd[clyr]; ilyr++) {
          minerl[(iel)*CENTMAXLYR+(clyr)] += nitrate[ilyr];
        }
      }
      minerl[(iel)*CENTMAXLYR+(*nlayer)] = nitrate[layers->numlyrs];
